...
# reducers
Redux.reducer(s::Vector{State}, a::AddTodo) = State[s..., State(isempty(s) ? 1 : s[end].id + 1, false, a.text)]
Redux.reducer(s::Vector{State}, a::DeleteTodo) = filter(s -> s.id !== a.id, s)
function Redux.reducer(s::Vector{State}, a::EditTodo)
    map(s) do s
        s.id === a.id ? State(s.id, s.completed, a.text) : s
    end
end
function Redux.reducer(s::Vector{State}, a::CompleteTodo)
    map(s) do s
        s.id === a.id ? State(s.id, !s.completed, s.text) : s
    end
end
function Redux.reducer(s::Vector{State}, a::CompleteAllTodos)
    are_all_marked = all(x->x.completed, s)
    map(x->State(x.id, !are_all_marked, x.text), s)
end
Redux.reducer(s::Vector{State}, a::ClearCompleted) = filter(s -> s.completed == false, s)
